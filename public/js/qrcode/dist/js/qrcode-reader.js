;(function( $, window, document, undefined ) {
  ...
  QRCodeReader.prototype = {
    ...
    buildHTML: function() {
      ...
      qrr.closeBtn = $(`
        <button type="button" id="qrr-close" class="btn btn-danger waves-effect">
          Close
        </button>
      `)

      // qrr.closeBtn.appendTo('.scanner');

      qrr.okBtn = $(`
        <button type="button" id="qrr-ok" class="btn btn-success waves-effect">
          Scan!
        </button>
      `)
      ...
      qrr.outputNoData.appendTo('.scanner > div > div:nth-child(2)');
      qrr.outputData.appendTo(qrr.outputDiv);
      
      qrr.loadingMessage.appendTo('.scanner > div > div:first');
      qrr.canvas.appendTo('.scanner > div > div:first');
      qrr.outputDiv.appendTo('.scanner > div > div:nth-child(2)');
      qrr.audio.appendTo('.scanner > div > div:first');
      qrr.okBtn.appendTo('.scanner > div > div:first');
    ...
    open: function () {
      ...
      if (qrr.settings.multiple) {
        ...
        qrr.okBtn.hide();
        qrr.qrCodeSubmit.appendTo('.scanner')
      }
    ...
    read: function() {
      ...
      if (qrr.video.readyState === qrr.video.HAVE_ENOUGH_DATA) {
        ...
        if (code && qrr.settings.qrcodeRegexp.test(code.data)) {
          ...
          $('#qrr-output-data').html(JSON.stringify(JSON.parse(code.data), null, 4))
